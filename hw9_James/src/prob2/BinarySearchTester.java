...
public class BinarySearchTester {
	...
	public static int binarySearch( ArrayList<Blob> blobs, Blob bKey1, Blob bKey2 ) {
		return binarySearch1(blobs, bKey1, bKey2, 0, blobs.size() - 1);
		}
	
	private static int binarySearch1( ArrayList<Blob> blobs, Blob bkey1, Blob bKey2, int low, int high) {
		if( low > high ) // key not found
			return -(low+1);
		else {
			int mid = (low+high)/2;

			if( bkey1 < blobs.get(mid)) { // search left sub-list
				return binarySearch1( blobs, bkey1, bKey2, low, mid-1 );
			}
			else if( bkey1 > blobs.get(mid) ) // search right sub-list
				return binarySearch1( blobs, bkey1, bKey2, mid+1, high );
